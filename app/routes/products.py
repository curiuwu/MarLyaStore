...
@login_required
def checkout():
    """Atomic checkout: wallet debit, order, items, status and cart cleanup."""
    try:
        cart_items = Cart.query.filter_by(user_id=current_user.id).all()
        if not cart_items:
        ...
        if not status:
            db.session.rollback()
            flash("Статус заказа не найден. Запустите seed-db.", "danger")
            return redirect(url_for("products.cart"))
        ...
        if not wallet:
            db.session.rollback()
            flash("Кошелёк не найден", "danger")
            return redirect(url_for("products.cart"))
        ...
        for item in cart_items:
            if not item.product:
                db.session.rollback()
                flash("В корзине найден недоступный товар", "danger")
                return redirect(url_for("products.cart"))

            quantity = int(item.quantity or 0)
            if quantity < 1:
                db.session.rollback()
                flash("Некорректное количество товара в корзине", "danger")
                return redirect(url_for("products.cart"))

            total_cost += Decimal(str(item.product.price)) * quantity
